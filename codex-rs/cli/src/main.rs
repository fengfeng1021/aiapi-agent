...
#[clap(
    ...
    // Aiapi Agent - was codex, now aiapi
    bin_name = "aiapi",
    override_usage = "aiapi [OPTIONS] [PROMPT]\n       aiapi [OPTIONS] <COMMAND> [ARGS]"
)]
struct MultitoolCli {
    #[clap(flatten)]
...
enum Subcommand {
    ...
    /// Launch Aiapi Agent Desktop (opens installer if missing).
    #[cfg(any(target_os = "macos", target_os = "windows"))]
    #[command(name = "agent", alias = "app")]
    Agent(app_cmd::AppCommand),
