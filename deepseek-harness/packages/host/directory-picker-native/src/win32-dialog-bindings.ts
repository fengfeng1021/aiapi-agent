...
// COINIT_APARTMENTTHREADED (0x2) | COINIT_DISABLE_OLE1DDE (0x4) avoids legacy DDE broadcast delays during drag.
const COINIT_APARTMENTTHREADED = 0x2 | 0x4
...
export async function loadWin32DialogBindings(): Promise<Win32DialogBindings> {
  ...
  const openDesktop = user32.func('__stdcall', 'OpenDesktopW', 'void *', ['str16', 'int32', 'bool', 'uint32'])
  const setThreadDesktop = user32.func('__stdcall', 'SetThreadDesktop', 'bool', ['void *'])
  try {
    const hDesk = openDesktop('default', 0, false, 0x1ff)
    if (hDesk !== null) {
  ...
  const setPriorityClass = kernel32.func('__stdcall', 'SetPriorityClass', 'bool', ['void *', 'uint32'])
  try {
    setPriorityClass(getCurrentProcess(), 0x80) // HIGH_PRIORITY_CLASS
  } catch {}

  try {
    const setProcessDpiAwarenessContext = user32.func('__stdcall', 'SetProcessDpiAwarenessContext', 'bool', ['intptr'])
