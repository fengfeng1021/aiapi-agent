...
import { CodexLoopAgent } from './codex-agent.ts'
...
export interface Config {
  /** Underlying agent loop engine ('codex' or 'react'). Defaults to 'codex'. */
  engine?: 'codex' | 'react'
  /**
...
export class AgentLoop extends Service implements AgentFactory {
  ...
  private prepare(ownerCtx: Context, id: SessionId, options: AgentOptions, session: Session, callerSignal?: AbortSignal): PreparedAgent {
    ...
    let machine: CodexLoopAgent | undefined
    ...
    try {
      const agent = machine = new CodexLoopAgent(loopCtx, id, options, session)
  ...
  private resumeWith(
    ...
    const published = (async () => {
      ...
      try {
        ...
        ownerCtx.fiber.assertActive()
        if (!this.ownership.isActive()) throw new Error('agent loop is not active')
        if (!preparation) throw new Error(`failed to prepare session "${id}"`)
...
export * from './param-sanitizer.ts'
export * from './responses-bridge.ts'
