...
import { EMPTY_RESPONSE_CODE, LlmError, StreamingThinkingNormalizer, extractWireReasoningDelta, repairJsonArguments } from '@deepseek-ai/dsh-llm'
...
function closeBlock(block: OpenBlock): ContentBlock {
  switch (block.kind) {
    ...
    case 'tool-call': return {
      ...
      name: block.name ?? '',
      arguments: repairJsonArguments(block.text),
    }
  }
}
...
export async function* translate(payloads: AsyncIterable<string>): AsyncGenerator<StreamChunk> {
  ...
  for await (const payload of payloads) {
    if (payload === DONE) {
      for (const part of normalizer.flush()) {
        if (part.type === 'reasoning') {
          ...
          yield { type: 'reasoning-delta', index: reasoningBlock.index, text: part.text }
        } else {
          if (!textBlock) {
            textBlock = open('text')
    ...
    for (const choice of chunk.choices ?? []) {
      ...
      // 1. Channel One: Check structural wire reasoning fields
      // (DeepSeek reasoning_content, OpenRouter reasoning, thinking, thought)
      const wireReasoning = extractWireReasoningDelta(choice)
      if (typeof wireReasoning === 'string' && wireReasoning.length > 0) {
        if (!reasoningBlock) {
          reasoningBlock = open('reasoning')
          yield { type: 'block-start', index: reasoningBlock.index, blockType: 'reasoning' }
        }
        reasoningBlock.text += wireReasoning
        yield { type: 'reasoning-delta', index: reasoningBlock.index, text: wireReasoning }
      }

      // 2. Channel Two: Content streaming with universal tag normalizer (<think>, <thought>, [THINK], etc.)
      const content = delta?.content
      if (typeof content === 'string' && content.length > 0) {
        const parts = normalizer.feedContent(content)
