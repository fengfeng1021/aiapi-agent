...
describe('translate: universal thinking normalization', () => {
  it('translates OpenRouter delta.reasoning into reasoning-delta', async () => {
    const chunks = await collect(translate(feed(
      firstChunk,
      { choices: [{ delta: { reasoning: 'analyzing problem via OpenRouter' } }] },
      { choices: [{ delta: { content: 'here is the solution' } }] },
      { choices: [{ delta: {}, finish_reason: 'stop' }] },
      DONE,
    )))
    ...
    expect(reasoning[0]).toEqual({ type: 'reasoning-delta', index: 0, text: 'analyzing problem via OpenRouter' })

    expect(text).toHaveLength(1)
    expect(text[0]).toEqual({ type: 'text-delta', index: 1, text: 'here is the solution' })
  })

  it('translates in-band <think>...</think> in delta.content into reasoning-delta and text-delta', async () => {
    const chunks = await collect(translate(feed(
      firstChunk,
      { choices: [{ delta: { content: '<think>Let me calculate</think>42' } }] },
      { choices: [{ delta: {}, finish_reason: 'stop' }] },
      DONE,
    )))
    ...
    expect(reasoning).toEqual([{ type: 'reasoning-delta', index: 0, text: 'Let me calculate' }])
    expect(text).toEqual([{ type: 'text-delta', index: 1, text: '42' }])
  })

  it('translates token-split <think> across multiple SSE payloads', async () => {
    const chunks = await collect(translate(feed(
      firstChunk,
      { choices: [{ delta: { content: '<th' } }] },
      { choices: [{ delta: { content: 'ink>planning steps' } }] },
      { choices: [{ delta: { content: '</th' } }] },
      { choices: [{ delta: { content: 'ink>Result' } }] },
      { choices: [{ delta: {}, finish_reason: 'stop' }] },
      DONE,
    )))
    ...
    expect(reasoning.map(r => (r as any).text).join('')).toBe('planning steps')
    expect(text.map(t => (t as any).text).join('')).toBe('Result')
  })
})
