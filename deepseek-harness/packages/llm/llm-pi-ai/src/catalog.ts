...
import { matchGlobalModelSpec } from './registry-sync.ts'
 ...
 */
interface ModelReasoningInference {
  reasoning: boolean
  thinkingLevelMap?: ThinkingLevelMap
}

/**
 * Infer model reasoning capabilities and thinking levels by model family signature.
 * Adapts to market standards (OpenAI o-series, GPT-5/Luna, Claude 3.7+, DeepSeek R1,
 * Gemini 2.0/2.5, Qwen QwQ, Grok) without blindly applying one fixed mapping to everything.
 */
function isExplicitNonReasoningModel(text: string): boolean {
  ...
  return (
    ...
    || lower.includes('claude-3-opus-2024')
...
function inferModelReasoning(id: string, name?: string, _provider?: string): ModelReasoningInference {
  const combined = `${id} ${name ?? ''}`.toLowerCase()

  if (isExplicitNonReasoningModel(combined)) {
    return { reasoning: false }
  }

  // 0. Prioritize live global registry match if available
  const globalSpec = matchGlobalModelSpec(id, null)
  if (globalSpec?.reasoning?.supportedEfforts && globalSpec.reasoning.supportedEfforts.length > 0) {
    const supported = new Set(globalSpec.reasoning.supportedEfforts.map(e => e.toLowerCase()))
    ...
    return {
      reasoning: true,
      thinkingLevelMap: map as ThinkingLevelMap,
    }
  }

  // 1. OpenAI o1 / o1-mini Family (strictly thinking, no 'off', low/medium/high)
  if (
    /(?:^|[\/_\-\s])(?:o1)(?:[\/_\-\s]|$)/i.test(combined)
    && !combined.includes('gpt-5')
    && !combined.includes('gpt-6')
  ) {
    return {
      reasoning: true,
      thinkingLevelMap: {
  ...
  // 2. OpenAI o3 / o3-mini / o4 Family (strictly thinking, no 'off', low/medium/high/max)
  if (
    /(?:^|[\/_\-\s])(?:o3|o4|o5)(?:[\/_\-\s]|$)/i.test(combined)
    && !combined.includes('gpt-5')
    && !combined.includes('gpt-6')
  ) {
    return {
  ...
  // 3. Anthropic Claude Opus 5 / UltraCode Family (off, low, medium, high, xhigh, max)
  if (
    combined.includes('opus')
    || combined.includes('ultracode')
    || combined.includes('claude-5')
  ) {
    return {
      reasoning: true,
      thinkingLevelMap: {
  ...
  // 4. Anthropic Claude 3.7+ / Claude 4 Family (off, low, medium, high, max)
  if (
    combined.includes('claude-3-7')
    || combined.includes('claude-3.7')
  ...
  // 6. Google Gemini 2.0 / 2.5 / 3.0 Flash Thinking / Pro Family (off, low, medium, high, max)
  if (
    combined.includes('gemini-2')
    || combined.includes('gemini-3')
    || combined.includes('flash-thinking')
  ) {
    return {
      reasoning: true,
      thinkingLevelMap: {
  ...
  // 7. xAI Grok 3 / 4 Reasoning Family (low, medium, high - no off)
  if (
    combined.includes('grok-3')
    || combined.includes('grok-4')
  ) {
    return {
      reasoning: true,
      thinkingLevelMap: {
  ...
  // 8. Qwen QwQ / Qwen3 / Qwen4 Family (off, low, medium, high)
  if (combined.includes('qwq') || combined.includes('qwen3') || combined.includes('qwen4')) {
    return {
      reasoning: true,
      thinkingLevelMap: {
        minimal: null,
  ...
  // 9. Universal / Future Frontier Models (Default Universal Reasoning: off, low, medium, high, max)
  return {
    reasoning: true,
    thinkingLevelMap: {
      minimal: null,
...
function resolveModelReasoning(
  ...
  if (efforts === undefined) {
    ...
    const inferred = inferModelReasoning(entry.id, entry.name, provider)
    if (inferred.reasoning) {
      return inferred.thinkingLevelMap !== undefined
        ? { reasoning: true, thinkingLevelMap: inferred.thinkingLevelMap }
        : { reasoning: true }
    }
  ...
  if (efforts && typeof efforts === 'object') {
    const rawMap = efforts as Record<string, string | null>
    if ('none' in rawMap) {
      if (rawMap.off === undefined) {
        rawMap.off = rawMap.none === 'none' ? null : rawMap.none
      }
      delete rawMap.none
    }
  }
