 ...
 */
export function repairJsonArguments(raw: string): string {
  ...
  if (text.includes("'")) {
    ...
    const singleQuoteReplaced = text.replace(
      /(?<=[:,\{\[\s])'((?:\\.|[^'\\])*)'(?=[:,\}\]\s]|$)/g,
      '"$1"'
    )
    try {
 ...
 */
export function extractInBandToolCalls(content: string): InBandToolCallExtractionResult {
  if (!content) return { cleanText: '', toolCalls: [] }

  const toolCalls: ExtractedInBandToolCall[] = []
  ...
  const cleanText = content.replace(tagRegex, (_match, body: string) => {
