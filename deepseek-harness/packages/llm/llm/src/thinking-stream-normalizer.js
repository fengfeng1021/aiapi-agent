 ...
 */
export const KNOWN_THINKING_TAGS = ['think', 'thinking', 'thought', 'reasoning'];
export class StreamingThinkingNormalizer {
    mode = 'text';
    buffer = '';
    justClosedThinking = false;
    tagNames;
    constructor(options) {
    ...
    isTagPrefix(str, isBracket) {
        const maxTagLen = 14; // e.g. '</reasoning>' is 12
        if (str.length > maxTagLen)
            return false;
        const openChar = isBracket ? '[' : '<';
        if (!str.startsWith(openChar))
            return false;
 ...
 */
export function extractWireReasoningDelta(choiceOrDelta) {
    if (!choiceOrDelta || typeof choiceOrDelta !== 'object')
        return undefined;
    const obj = choiceOrDelta;
    const delta = obj.delta && typeof obj.delta === 'object' ? obj.delta : obj;
    const candidate = delta.reasoning_content ??
        delta.reasoning ??
        delta.thinking ??
        delta.thought ??
...
//# sourceMappingURL=thinking-stream-normalizer.js.map