...
export interface Config {
  ...
  model?: string
  /** Model provider name (e.g. `opencode-go`). */
...
type ResolvedConfig = Omit<Required<Config>, 'model' | 'modelProvider'> & Pick<Config, 'model' | 'modelProvider'>
...
class CodexProvider implements SubagentProvider {
  ...
  start(request: ResolvedSubagentStartRequest) {
    ...
    const spec: CodexRunSpec = {
      ...
      ...this.config.modelProvider === undefined ? {} : { modelProvider: this.config.modelProvider },
      permissionMode: this.config.permissionMode,
      env: {
        ...this.config.env,
        CODEX_HOME: process.env.CODEX_ISOLATED_HOME || join(homedir(), '.dsh', 'codex-isolated'),
      },
