...
export function NativeDirectoryFlow(props: DirectoryFlowOwnerProps & NativeFlowInjected): ReactElement | null {
  ...
  useEffect(() => {
    ...
    console.log('[NativeDirectoryFlow] mounted, open is:', open)
    alive.current = true
    return () => { alive.current = false }
  }, [])
  useEffect(() => {
    console.log('[NativeDirectoryFlow] effect triggered, open:', open, 'armed:', armed.current)
    if (!open) {
      armed.current = false
      return
    }
    ...
    armed.current = true
    console.log('[NativeDirectoryFlow] calling pick()...')
    pick().then(
      (path) => {
        console.log('[NativeDirectoryFlow] pick() resolved with path:', path)
        if (!alive.current) return
        if (path === null) outcome.current.onCancel(); else outcome.current.onPicked(path)
      },
      (reason: unknown) => {
        console.error('[NativeDirectoryFlow] pick() rejected with:', reason)
        if (!alive.current) return
