...
describe('ModelSelect reasoning effort', () => {
  it('renders effort names without descriptions and submits the effort as part of the session selection', async () => {
    ...
    const trigger = screen.getByRole('button', {
      name: '选择模型，当前 DeepSeek-V4-Flash，推理程度 超高',
    })
    fireEvent.click(trigger)
    fireEvent.click(screen.getByRole('menuitem', { name: /推理程度/ }))
    expect(screen.getAllByRole('menuitemradio').map(item => item.textContent))
      .toEqual(['關閉', '高', '超高更快消耗使用額度'])
    ...
    await waitFor(() => {
      ...
      expect(trigger.getAttribute('aria-label')).toBe('选择模型，当前 DeepSeek-V4-Flash，推理程度 高')
    })
  })

  it('defaults to highest reasoning effort and lists model efforts', () => {
    ...
    fireEvent.click(screen.getByRole('button', {
      name: '选择模型，当前 Model，推理程度 Standard',
    }))
    fireEvent.click(screen.getByRole('menuitem', { name: /推理程度/ }))
    expect(screen.getAllByRole('menuitemradio').map(item => item.textContent))
      .toEqual(['Standard'])
