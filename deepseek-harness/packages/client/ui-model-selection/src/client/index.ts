...
function getHighestEffort(reasoning?: { efforts: readonly { id: string }[] }): string | undefined {
  if (!reasoning || !reasoning.efforts || reasoning.efforts.length === 0) return undefined
  ...
  const nonOff = reasoning.efforts.filter(e => e.id.toLowerCase() !== 'off')
  if (nonOff.length === 0) return reasoning.efforts[0]?.id
  ...
  for (const group of state.groups) {
    for (const model of group.models) {
      ...
      const reasoningEffort = sameRoute
        ? state.current?.reasoningEffort ?? highest
        : highest
      return {
        provider: group.id,
        model: model.id,
