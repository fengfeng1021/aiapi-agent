...
/**
 * Minimal structural view of a stream chunk for first-token detection.
 * Mirrors the `StreamChunk` variants this fold inspects; extra fields on the
 * real chunk type are ignored.
 */
export interface TokenDeltaChunk {
...
/**
 * Whether a stream chunk carries visible model output for timing.
 * Local copy of the predicate formerly imported from `@deepseek-ai/dsh-llm/message`
 * (removed upstream): client bundle purity forbids cross-plugin value imports,
 * so the runtime object layer owns this 10-line leaf. Keep in sync with the
 * same predicate in `ui-chat`'s event projection.
 * @param chunk - Stream chunk to inspect.
 * @returns true for a non-empty text, reasoning, or tool-call delta.
 */
export function isTokenDelta(chunk: TokenDeltaChunk): boolean {
  switch (chunk.type) {
    case 'text-delta':
