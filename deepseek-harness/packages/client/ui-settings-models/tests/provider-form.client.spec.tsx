...
function scriptedFace(options: {
  ...
  /** Custom list of configurable provider ids for directory testing. */
  configurableProviders?: readonly string[]
  ...
  const face = {
    llm: {
      ...
      listConfigurableProviders: vi.fn(() => Promise.resolve(ok(
        (options.configurableProviders ?? Object.keys(providers)).map(provider => ({
          provider,
...
describe('endpoint interrogation', () => {
  ...
  it('adopts only the picked candidates, keeping a row the user already tuned', async () => {
    ...
    fireEvent.click(screen.getByText(en.fetchModels))
    await screen.findByText(en.fetchAdopt)
    // The already-configured row starts checked.
...
describe('API key field', () => {
  ...
  it('displays OpenCode provider presets and multi-login options when adding provider', async () => {
    await mountSection({
      providers: {},
      configurableProviders: ['openai', 'openai-codex', 'anthropic', 'google', 'deepseek'],
      declaredRoutes: [],
    })

    // Click "浏览全部提供商..." (Browse all providers)
    fireEvent.click(screen.getByText('浏览全部提供商...'))

    // Expect OpenCode preset buttons to be present
    expect(screen.getByText('热门')).toBeTruthy()
    expect(screen.getByPlaceholderText('搜索提供商')).toBeTruthy()
    expect(screen.getAllByText('OpenAI').length).toBeGreaterThan(0)
    expect(screen.getAllByText('Anthropic').length).toBeGreaterThan(0)

    // Click OpenAI preset button
    const openAiBtn = screen.getAllByRole('button').find(btn => btn.textContent?.includes('OpenAI') && btn.className.includes('pickerItemBtn'))!
    fireEvent.click(openAiBtn)

    // Expect OpenCode login options matching media_1788327891145.png
    expect(screen.getByText('选择 OpenAI 的登录方式。')).toBeTruthy()
    expect(screen.getAllByText('ChatGPT Pro/Plus').length).toBeGreaterThan(0)
    expect(screen.getByText('无头模式')).toBeTruthy()
    expect(screen.getByText('API 密钥')).toBeTruthy()

    // Click "ChatGPT Pro/Plus" (OAuth browser mode matching media_1788327902554.png)
    const proPlus = screen.getAllByText('ChatGPT Pro/Plus')[0]
    expect(proPlus).toBeTruthy()
    if (proPlus !== undefined) fireEvent.click(proPlus)
    expect(screen.getByText('确认码')).toBeTruthy()
    expect(screen.getByText(/Complete authorization/)).toBeTruthy()
