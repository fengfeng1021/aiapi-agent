...
export function ProviderEditor(props: ProviderEditorProps): ReactNode {
  ...
  const defaultProviderBaseUrl = props.provider === 'opencode-go' || props.provider?.includes('opencode')
    ? 'https://opencode.ai/zen/go/v1'
    : (props.provider === 'aiapi' ? 'https://aiapi.tw' : undefined)
  const probeApi = stringAt(draft, 'api') ?? stringAt(fallback, 'api')
  const probeBaseURL = stringAt(draft, 'baseURL') ?? stringAt(fallback, 'baseURL') ?? defaultProviderBaseUrl
  const probe = {
          ...
          <div className={styles['customizedBody']}>
            ...
            <div className={styles['field']}>
              ...
              <input
                className={styles['input']}
                type="text"
                value={props.provider === 'aiapi' ? 'https://aiapi.tw' : stringAt(draft, 'baseURL') ?? ''}
                placeholder={props.provider === 'aiapi'
                  ? 'https://aiapi.tw'
                  : family === 'deepseek'
                    ? DEEPSEEK_PUBLIC_BASE_URL
                    : stringAt(fallback, 'baseURL') ?? t('baseUrlDefault')}
                aria-label={t('baseUrl')}
                disabled={disabled || props.provider === 'aiapi'}
                onChange={(event) => {
                  setField('baseURL', event.target.value === '' ? undefined : event.target.value)
                }}
              />
              {props.provider === 'aiapi' && (
