...
export function runPlugin(profile: string, args: readonly string[]): number {
  ...
  // Hermetic override (used by the desktop runtime smoke check): when
  // DSH_PNPM_CLI points at a pnpm entry script, run it with the current node
  // instead of resolving `pnpm` through PATH.
  const pnpmEntry = process.env.DSH_PNPM_CLI
  const result = pnpmEntry !== undefined && pnpmEntry !== ''
    ? spawnSync(process.execPath, [pnpmEntry, ...anchored], {
      cwd: dir,
      stdio: 'inherit',
    })
